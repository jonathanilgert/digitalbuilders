...
import { HoursGrid } from "@/components/portal/hours-grid";
...
const templates: TemplateOption[] = [
  {
    ...
    image: "/portal/templates/classic.png",
    layout: "classic",
  },
  {
    id: "bold",
    ...
    image: "/portal/templates/bold.png",
    layout: "bold",
  },
  {
    id: "craftsman",
    ...
    image: "/portal/templates/craftsman.png",
    layout: "craftsman",
  },
  {
    id: "response",
    ...
    image: "/portal/templates/response.png",
    layout: "response",
  },
  {
    id: "modern",
    ...
    image: "/portal/templates/modern.png",
    layout: "modern",
  },
];
...
function MiniTemplate({ template, selected }: { template: TemplateOption; selected: boolean }) {
  return (
    <div className={`mt-4 overflow-hidden rounded-2xl border ${selected ? "border-accent" : "border-line"} bg-ink shadow-inner`} aria-hidden="true">
      <div className="relative h-64 overflow-hidden bg-surface sm:h-72">
        <Image
          src={template.image}
          alt=""
          fill
          ...
          loading="lazy"
        />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-ink/85 to-transparent" />
      </div>
    </div>
  );
}
...
export function StepForm({ number, initial, state, assets = [] }: Props) {
  ...
  return <div className="space-y-5">
    ...
    {number === 5 && <div className="space-y-4"><HoursGrid data={data} onChange={update} /><label className="block text-sm text-fg-muted"><input type="checkbox" checked={Boolean(data.emergency_24_7)} onChange={e=>update("emergency_24_7",e.target.checked)} /> 24/7 emergency line</label><label className="block text-sm text-fg-muted"><input type="checkbox" checked={Boolean(data.by_appointment)} onChange={e=>update("by_appointment",e.target.checked)} /> By appointment only</label><textarea className={cls} rows={3} placeholder="Seasonal hours or notes" value={str(data,"hours_note")} onChange={e=>update("hours_note",e.target.value)} /></div>}
